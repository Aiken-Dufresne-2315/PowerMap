    ...
    int optimizeEdgeOrientation(std::vector<BaseVertexProperty>& vertexList, std::vector<BaseEdgeProperty>& edgeList, BaseUGraphProperty& graph) {
        try {
            ...
            for (int e = 0; e < edgeNum; ++e) {
                ...
                // GRBQuadExpr edgeLengthExpr = (X[i] - X[j]) * (X[i] - X[j]) + (Y[i] - Y[j]) * (Y[i] - Y[j]);
                // model.addQConstr(edgeLengthExpr >= EDGE_LENGTH_THRESHOLD * EDGE_LENGTH_THRESHOLD, "edge_length_" + std::to_string(e));
            }
